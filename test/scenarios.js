...
contract('TollBoothOperator', function (accounts) {
  ...
  describe("Vehicle Operations", function () {
    ...
    beforeEach("should deploy regulator and operator", function () {
      return Regulator.new({
        ...
        .then(() => regulator.setVehicleType(vehicle0, vehicleType0, {
          from: owner0
        }))
        .then(tx => regulator.setVehicleType(vehicle1, vehicleType1, {
          from: owner0
        }))
        .then(tx => regulator.createNewOperator(owner1, deposit0, {
          from: owner0
        }))
        .then(tx => operator = TollBoothOperator.at(tx.logs[1].args.newOperator))
        .then(() => operator.addTollBooth(booth0, {
          from: owner1
        }))
        .then(tx => operator.addTollBooth(booth1, {
          from: owner1
        }))
        .then(tx => operator.addTollBooth(booth2, {
          from: owner1
        }))
        .then(tx => operator.setMultiplier(vehicleType0, multiplier0, {
          from: owner1
        }))
        .then(tx => operator.setMultiplier(vehicleType1, multiplier1, {
          from: owner1
        }))
        .then(tx => operator.setRoutePrice(booth0, booth1, price01, {
          from: owner1
        }))
        .then(tx => operator.setPaused(false, {
          from: owner1
        }))
  ...
  describe("reportExitRoad with required amount deposited", function () {

    let vehicleInitBal;

    beforeEach("should enter road with required amount deposit", function () {
      return operator.enterRoad(
          booth0, hashed0, {
            from: vehicle0,
            value: (deposit0 * multiplier0)
          })
    });

    it("should be possible to report exit road on route with known price equal to deposited", function () {
      return operator.setRoutePrice(booth0, booth1, deposit0, {
          from: owner1
        })
        .then(tx => operator.reportExitRoad.call(secret0, {
          from: booth1
        }))
        .then(result => assert.strictEqual(result.toNumber(), 1))
        .then(() => operator.reportExitRoad(secret0, {
          from: booth1
        }))
        .then(tx => {
    ...
    it("should be possible to report exit road on route with known price greater than deposited", function () {
      return operator.setRoutePrice(booth0, booth1, deposit0, {
          from: owner1
        })
        .then(tx => operator.reportExitRoad.call(secret0, {
    ...
    it("should be possible to report exit road on route with known price less than deposited", function () {
      return operator.setRoutePrice(booth0, booth1, (deposit0 - 1), {
          from: owner1
        })
        .then(tx => operator.reportExitRoad.call(secret0, {
          from: booth1
        ...
        .then(tx => {
          ...
          assert.strictEqual(logExited.args.refundWeis.toNumber(), 1*multiplier0);
        });
    });
  ...
  describe("reportExitRoad with excessive deposited", function () {
    ...
    beforeEach("should enter road with excessive deposit", function () {
      return operator.enterRoad(
          booth0, hashed0, {
            from: vehicle0,
            value: (deposit0 + extraDeposit) * multiplier0
          })
    ...
    it("should be possible to report exit road on route with unknown price", function () {
      return operator.reportExitRoad.call(secret0, {
          from: booth2
        })
        ...
        .then(() => operator.setRoutePrice.call(booth0, booth2, deposit0, {
            from: owner1
          })
          .then(success => assert.isTrue(success))
          .then(() => operator.setRoutePrice(booth0, booth2, deposit0, {
            from: owner1
          }))
          .then(tx => {
  ...
  describe("Pending payments with vehicles on same route, then setRoutePrice", function () {
    ...
    beforeEach("should have 2 vehicles enter and exit road on same unknown route. Vehicle 1 deposits more than required, vehicle to exact the required amount", function () {
      return operator.enterRoad(
          booth0, hashed0, {
            from: vehicle0,
            value: extraDeposit0 * multiplier0
          })
        .then(tx => operator.enterRoad(
          booth0, hashed1, {
            from: vehicle1,
            value: deposit0 * multiplier1
          }))
        .then(tx => web3.eth.getBalancePromise(vehicle0))
        .then(balance => vehicle0InitBal = balance)
        .then(() => web3.eth.getBalancePromise(vehicle1))
        .then(balance => vehicle1InitBal = balance)
        .then(() => operator.reportExitRoad(secret1, {
          from: booth2
        }))
        .then(tx => operator.reportExitRoad(secret0, {
          from: booth2
        }));
    });

    it("should be possible to set the base route price below the required deposits and both get refunded", function () {
      return operator.setRoutePrice.call(booth0, booth2, (deposit0-1), {
          from: owner1
        })
        .then(success => assert.isTrue(success))
        .then(() => operator.setRoutePrice(booth0, booth2, (deposit0-1), {
          from: owner1
        }))
        .then(tx => {
          ...
          assert.strictEqual(logPriceSet.args.entryBooth, booth0);
          assert.strictEqual(logPriceSet.args.exitBooth, booth2);
          assert.strictEqual(logPriceSet.args.priceWeis.toNumber(), deposit0-1);
          const logExited = tx.logs[1];
          assert.strictEqual(logExited.event, "LogRoadExited");
          assert.strictEqual(logExited.args.exitBooth, booth2);
          assert.strictEqual(logExited.args.exitSecretHashed, hashed1);
          assert.strictEqual(logExited.args.finalFee.toNumber(), (deposit0-1) * multiplier1);
          assert.strictEqual(
            logExited.args.refundWeis.toNumber(),
            (deposit0 - (deposit0-1)) * multiplier1);
        })
        .then(() => operator.clearSomePendingPayments(booth0, booth2, 1, {
            from: owner0
          }))
          .then(tx => {
            assert.strictEqual(tx.receipt.logs.length, 1);
            ...
            assert.strictEqual(
              logExited.args.refundWeis.toNumber(),
              ((extraDeposit0 - (deposit0-1)) * multiplier0));
